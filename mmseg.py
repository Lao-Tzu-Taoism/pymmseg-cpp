...
mmseg = cdll.LoadLibrary(join(dirname(__file__), 'mmseg-cpp', 'mmseg.%s' % ext))
...
class Token(Structure):
    _fields_ = [('_text', c_void_p), ('_offset', c_int), ('_length', c_int)]

    def text_get(self) -> bytes:
    ...
    def __repr__(self):
        return '<Token %d..%d %s>' % (self.start, self.end,
                                      self.text.__repr__())
...
def dict_load_defaults():
    mmseg.mmseg_load_chars((join(dirname(__file__), 'data',
                                 'chars.dic')).encode('utf8'))
    mmseg.mmseg_load_words((join(dirname(__file__), 'data',
                                 'words.dic')).encode('utf8'))
...
class Algorithm(object):
    def __init__(self, text: str):
        ...
        self.text = text.encode(
            'utf8')  # add a reference to prevent the string buffer from
        # being GC-ed

        self.algor = mmseg.mmseg_algor_create(self.text, len(self.text))
